...
- **2 つのビルドジョブ**を持ちます。`build`（`macos-15`・Apple Silicon、Xcode 16.2）と
  `build-windows`（`windows-latest`・Visual Studio 2022）です。それぞれが対応する
  プラットフォームの SDK をダウンロードします。
...
- **ダウンロード可能なリリースを公開**し、アップデータが取得できる安定した URL を用意
  ...
  リリースは macOS ジョブ（`build-mac`）が作成します。Windows ジョブ（`build-windows`）は
  macOS ジョブと**並行**して走り、自分の `.vlb.zip` アセットを同じリリースに**追加**します。
  両者は並行に走るため、Windows のアップロード手順はリリースが現れるのを待ってから
  アセットを追加します（まだ macOS ジョブがビルド中でリリースが存在しないだけ、という
  ケースに対応するため）。どちらもローリング方式で、毎回タグを最新
