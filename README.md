...
src/
  ModuleMain.cpp            モジュールのエントリポイント。拡張機能を登録し、
                            stable では起動時アップデート確認を仕掛ける
  Extensions/ExtMenu.{h,cpp}  アラートを表示するメニューコマンド（dev では
                            実行時にブランチ選択アップデートを起動）
  Updater.{h,cpp}           同梱した vw-update.sh を起動してアップデートを駆動する
...
scripts/
  vw-update.sh              CI ビルドをダウンロード／インストールする（バンドルに
                            同梱され、プラグインから起動される）
...
- **dev（`SamplePluginDev` / ブランチ）** — **コマンド実行時**に、使用するビルドを
  問い合わせます（`src/Extensions/ExtMenu.cpp` の `DoInterface`）。まず現在ロードされて
  いるビルド（branch / commit、「インストール済み」と明示）を提示し、他のブランチの
  プレリリース（現在のビルドは除外）があれば切り替え候補として選べます。
  - **現在のビルドを実行**を選ぶ → そのままプラグインのコマンドを実行します。
  - **別のブランチのビルドを選ぶ** → それをインストールしてコマンドは実行せず終了
    します（開発時はすぐ再起動して読み込み直したいため）。インストール完了メッセージを
    表示します。
...
されるのは次回 Vectorworks を起動（または手動で再起動）したときです。バンドルの隔離
解除とアドホック再署名はスクリプトが行います。

プラグイン経由の更新は、**実行中のバンドル自身が置かれているフォルダ**（＝Vectorworks
が実際に読み込んだ `Plug-Ins`）へインストールします（`src/Updater.cpp` が `dladdr` で
自分のパスを解決し、`VW_PLUGINS_DIR` としてスクリプトに渡します）。ユーザフォルダを
既定と違う場所に設定していても、読み込まれているコピーを直接置き換えるので更新が確実に
反映されます。なお手動 CLI（下記）は既定パスを使うため、ユーザフォルダが独自の場合は
`VW_PLUGINS_DIR` を実際の場所に合わせて実行してください。
