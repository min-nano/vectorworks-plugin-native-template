...
- **dev（`SamplePluginDev` / ブランチ）** — **コマンド実行時**に、使用するビルドを
  **ネイティブのプルダウンダイアログ**（`VWFC::VWUI::VWDialog` + `VWPullDownMenuCtrl`、
  `src/Updater.cpp` の `CBuildPickerDialog`）で問い合わせます。1 つのドロップダウンに
  候補を一覧表示します:
  - 先頭は**現在ロードされているビルド**（branch / commit、「インストール済み」と明示）。
  - 続いて**他のブランチのプレリリース**（現在のビルドと同じコミットは除外）。
  - **インストール済み（先頭）を選ぶ** → そのままプラグインのコマンドを実行します。
  - **別のブランチを選ぶ** → それをインストールしてコマンドは実行せず終了します
    （開発時はすぐ再起動して読み込み直したいため）。インストール完了メッセージを表示します。
